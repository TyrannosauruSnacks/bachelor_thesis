...
be aware that camtraR functions don't like `~` as path to directory
```{r}
...
Maybe a script can do that aswell. Bur preferably it would be done ad hoc
...
I did this manually with DigiKam, because camtrapR uses the original Time for filenames.
...
### 2.1.1 Alrighty, let's create the timeshift table for `deployment_01`
```{r}
timeshift_deployment_01 <- cameras |>
  filter(id_deployment == 1) |>
  select(!id_deployment:model) |>
  mutate(
    timeshift = if_else(camera_name == "e1", "0:0:0 3503:0:0",
          if_else(daylight_saving_time == "0", "0:0:0 1:0:0", NA)
          ),
    sign = if_else(camera_name == "e1","-",
        if_else(daylight_saving_time == "0", "+", NA)
        )
    )|>
...
```{r}
# timeshift p01
timeshift_run_01 <-timeShiftImages(
  inDir = indir_deployment_01,
...
# timeshift p02
timeshift_run_02 <- timeShiftImages(
  inDir = indir_deployment_02,
  hasCameraFolders = TRUE,
...
```{r}
renaming_table_p01 <- imageRename(inDir = indir_deployment_01,
...
renaming_table_p02 <- imageRename(inDir = indir_deployment_02,
...
leave timezone UTC, as Niedballa explained:
"[...]it is advised to set argument `timeZone` to your study area’s time zone (one of OlsonNames()),
unless the time zone of your study area has DST, but your cameras don’t record it."

```{r}
events_frames_01 <- recordTable(
...
add deployment id
```{r}
events_frames_01 <- events_frames_01 |> 
  mutate(id_deployment = 1)

events_frames_02 <- events_frames_02 |> 
   mutate(id_deployment = 2)
```

bind tables together, filter for species `rotwild` and add `id_classifier`
```{r}
events_frames_rwi <- bind_rows(events_frames_01, events_frames_02) |>
